...
- [] 自動保存機能 (ストーリー構成、ワールドガイド、キャラクター、フラグ)
- [] 手動保存機能 (ストーリー)
...
- 自動保存機能 (ストーリー構成、ワールドガイド、キャラクター、フラグ):
  ユーザーがストーリー制作中に意識せずに作業に集中できるよう、「ストーリー構成」「ワールドガイド」「キャラクター」「フラグ」は自動保存機能を実装予定です。
...
- 手動保存機能(ストーリー):
  「ストーリー」は、小説投稿サイトへの初回投稿前に下書き段階でじっくり構成を作成・調整するユーザーも多く想定されるため、手動保存機能を実装予定です。
  ストーリー詳細画面に「保存」ボタンを設け、押下時にステータス（「下書き」「公開」）を含めた内容をサーバー側に保存します。
  保存時のステータスは enumカラムで管理する予定です。初期保存時は「下書き」ステータス、ユーザーがステータスを操作し「公開」ステータスに切り替えられるUIを作成予定です。
...
- フラグ作成・回収管理機能:
  ...
  チェック操作は「ストーリー構成」画面内でのみ実行可能とし、ユーザーはストーリー進行に応じて使用するフラグの状態（「作成」「回収」）を管理できる設計とします。
  「ストーリー構成」画面でチェック状態を変更すると、その状態は非同期(AJAX)で即時保存され、「フラグ一覧」画面でも同期された状態が表示されます。
...
- 参考テンプレート化機能(ストーリー構成に実装):
  物語制作の初心者やアイデア整理に悩むユーザーのために、「プロット」作成時に参考にできる「起承転結」や「三幕構成」など代表的なストーリー構成テンプレートを数種類用意します。
  しかし、娯楽小説は作者の創作意欲次第で無限に広がり、情報量も膨大になることを前提に本サービスを企画しています。そのため、テンプレートはあくまで「ガイドライン」として提供し、ユーザーが枠にとらわれず自由に編集・拡張できる設計とする予定です。
  また、「プロット」はデフォルトで白紙状態から始め、ユーザーの好みや創作スタイルに合わせた柔軟な使い分けが可能にする予定です。

- 構成チェック機能(ストーリー構成に実装):
  膨大な情報を整理しながら物語制作を進められるよう、「ストーリー構成チェック機能」を実装予定です。この機能は「ストーリー構成」に実装予定です。
  
  一つのストーリー毎に「ストーリー構成」を一つ作ることができ、「ストーリー構成」ではジャンル、テーマ、複数の章の追加ができます。章一つごとに作成済みの「ワールドタイプ」「キャラクター」「フラグ」から任意数選択でき、「プロット」を一つ書くことができます。
  ユーザーが章ごとに設定した「ワールドタイプ」「キャラクター」「フラグ」と、実際に記述した「プロット」の内容を自動で照合することで、記述漏れや一貫性・整合性の保持を支援します。
  ...
  チェックは章ごとに任意のタイミングで実行でき、チェック結果は簡潔な文面として画面上に表示。ユーザーは不足や矛盾を早期に把握し、作品の完成度向上や整合性維持に活用できます。
...
- 使用想定のgem
  ...
  ・kaminari : ページネーション用
  ・rails-i18n : 日本語化対応
  ・actiontext : リッチテキスト用（ストーリー構成）
  ・image_processing : リッチテキスト用（ストーリー構成）
  ・actioncable : 本リリース時のコミュニティ用
